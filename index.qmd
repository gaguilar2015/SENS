...
-   sens.sav - the base file containing all household-level items. The modules included at this level are
    -   Demography (M. 1)
    -   Food Security (M. 5)
    -   Mosquito Net (M. 6)
    -   WASH (M. 7)
...
## R Code: Preparation {#preparation}
...
Additionally, some objects that will later help in theming plots and graphs are defined. All plots will be created using the `ggplot2` package. it is not important to understand the below code, but for more information on colors and theming in R and ggplot2, [please see here](http://sape.inf.usi.ch/quick-reference/ggplot2/colour).
...
2.  The household and individual level data frames are merged\*, using `dplyr`'s `left_join,` into a single data frame named `sens_df`.
...
Now, we create some important derived variables that will be used throughout the analysis.
